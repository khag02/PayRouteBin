...
public abstract class BaseChannel extends Observable
    ...
    // The lock objects should be final, and never changed, but due to the clone()
    // method, they must be set there.
        ...
        try {
            if (socketFactory != null)
                ...
                if (localIface != null || localPort != 0) {
                    InetAddress addr = localIface == null ? InetAddress.getLocalHost()
    ...
    protected Socket newSocket(String[] hosts, int[] ports, LogEvent evt)
        ...
        if (s == null)
            throw new IOException("%s:%d %s (%s)".formatted(h, p,
                    Caller.shortClassName(lastIOException.getClass().getName()), Caller.info(1)));
        return s;
    }
    ...
    /**
     * Read some trailer data from this channel and optionally store it in the
     * incoming ISOMsg.
     *
    ...
    public Object clone() {
        try {
            ...
            // The lock objects must also be cloned, and the DataStreams nullified, as it
            // makes no sense
            // to use the new lock objects to protect the old DataStreams.
            // This should be safe as the only code that calls BaseChannel.clone() is
            // ISOServer.run(),
            // and it immediately calls accept(ServerSocket) which does a connect(), and
            // that sets the stream objects.
