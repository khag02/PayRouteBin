...
public class TransactionManager
    ...
    public void setConfiguration(Configuration cfg) throws ConfigurationException {
        ...
        try {
            activeSessionsGauge = MeterFactory.gauge(getServer().getMeterRegistry(), MeterInfo.TM_ACTIVE_SESSIONS,
                    Tags.of("name", getName()), BaseUnits.THREADS, activeSessions::get);
            transactionCounter = MeterFactory.counter(getServer().getMeterRegistry(), MeterInfo.TM_COUNTER,
                    Tags.of("name", getName()));
    ...
    protected void commit(int session, long id, Serializable context, List<TransactionParticipant> members,
            boolean recover, LogEvent evt, Profiler prof) {
    ...
    protected void abort(int session, long id, Serializable context, List<TransactionParticipant> members,
            boolean recover, LogEvent evt, Profiler prof) {
        for (TransactionParticipant p : members) {
            ParticipantParams pp = getParams(p);
            if (recover && p instanceof ContextRecovery cr) {
    ...
    protected int prepare(int session, long id, Serializable context, List<TransactionParticipant> members,
            Iterator<TransactionParticipant> iter, boolean abort, LogEvent evt, Profiler prof,
    ...
    protected void initStatusListeners(Element config) throws ConfigurationException {
        ...
        while (iter.hasNext()) {
            ...
            final TransactionStatusListener listener = (TransactionStatusListener) factory
                    .newInstance(QFactory.getAttributeValue(e, "class"));
    ...
    protected List<TransactionParticipant> initGroup(Element e)
        ...
        for (Element el : e.getChildren("participant")) {
            if (QFactory.isEnabled(el)) {
                ...
                getLog().warn("participant ignored (enabled='" + QFactory.getEnabledAttribute(el) + "'): "
                        + el.getAttributeValue("class") + "/" + el.getAttributeValue("realm"));
            }
        }
        return group;
    }
    ...
    /**
     * This method returns the number of sessions that can be started at this point
     * in time
     *
    ...
    /**
     * This method returns true if current session should stop working on more
     * messages
     *
     * @return
     */
    ...
    private void notifyStatusListeners(int session, TransactionStatusEvent.State state, long id, String info,
    ...
    private ParticipantParams getParams(TransactionParticipant p) {
        return Optional.ofNullable(params.get(p)).orElse(
                new ParticipantParams(p.getClass().getName(), 0L, 0L, Collections.emptySet(), Collections.emptySet(),
    ...
    private int pauseAndWait(Serializable context, int action) {
        if (context instanceof Pausable pausable)
            try {
    ...
    private int prepareOrAbort(TransactionParticipant p, long id, Serializable context, ParticipantParams pp,
            TriFunction<TransactionParticipant, Long, Serializable, Integer> preparationFunction) {
        int action;
    ...
    private void commitOrAbort(TransactionParticipant p, long id, Serializable context, ParticipantParams pp,
            TriConsumer<TransactionParticipant, Long, Serializable> preparationFunction) {
        if (context instanceof Context ctx && pp.isConstrained()) {
            Context c = ctx.clone(pp.requires.toArray(), pp.optional.toArray());
    ...
    private Serializable recover(ContextRecovery p, long id, Serializable context, ParticipantParams pp,
            boolean commit) {
        ...
        try {
            if (context instanceof Context ctx && pp.isConstrained()) {
                ...
                return (s instanceof Context rc) ? rc.clone(pp.provides.toArray()) : s;
            } else {
