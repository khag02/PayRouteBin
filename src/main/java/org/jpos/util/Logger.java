...
public class Logger implements LogProducer, Configurable {
    ...
    // String classLog = evt.getClassName() != null ? evt.getClassName() : "Logger";
    // if (source != null) {
    // logger = source.getLogger();
    // }
    // if (logger == null && !evt.isHonorSourceLogger()) {
    // logger = getLogger(Q2.LOGGER_NAME);
    // }

    // String timestamp = DateTimeFormatter.ofPattern("yyyy-MM-dd
    // HH:mm:ss.SSSSSSSSS")
    // .format(LocalDateTime.now());
    // String level = evt.getTag() != null ? evt.getTag().toUpperCase() : "SERVICE";
    ...
    // int logPrefixLen = timestamp.length() + 1 + level.length() + 2;
    // int remaining = totalWidth - logPrefixLen;
    // int eqEachSide = Math.max((remaining - midLen) / 2, 0);

    // String logstart = String.format(
    // "%s %s %s%s%s",
    // timestamp,
    // level,
    // "=".repeat(eqEachSide),
    // mid,
    // "=".repeat(Math.max(0, remaining - eqEachSide - midLen)));

    // // Only print logstart if not "Logger"
    // if (!"Logger".equals(classLog)) {
    // System.out.println(logstart);
    // }

    // for (int i = 0; i < payload.size(); i++) {
    // Object line = payload.get(i);
    // String logLine = String.format("%s %s %s",
    // timestamp, level, line.toString().trim());

    // System.out.println(logLine);

    // // If last line and not "Logger", print closing line
    // if (i == lastIndex && !"Logger".equals(classLog)) {
    // System.out.print(logstart);
    // }
    // }
    // }

    // System.out.println(); // Extra spacing for clarity

    // if (logger != null && logger.hasListeners()) {
    // for (Iterator<LogListener> it = logger.listeners.iterator(); it.hasNext();) {
    // try {
    // evt = it.next().log(evt);
