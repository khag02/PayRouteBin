...
public class QMUX
    ...
    protected boolean isNotifyEligible(ISOMsg msg) {
        ...
        try {
            ...
            // MTI is not standard 4 characters (eg. FSDISOMsg), then notification is
            // expected.
    ...
    public String getKey(ISOMsg m) throws ISOException {
        ...
        // **ATOM-CUSTOM**: Check IS LOGON request
        boolean isLogon = m.getMTI().startsWith("08");
        ...
        for (String f : k) {
            ...
            if (v != null) {
                ...
                // **ATOM-CUSTOM**: Add is logon condition to ignore field 11: STB do not
                // include in response
                if (!("11".equals(f) && isLogon)) {
                    sb.append(v);
                }
            }
        }
    ...
    private void addListeners() throws ConfigurationException {
        ...
        for (Element l : rlisten) {
            ISORequestListener listener = (ISORequestListener) factory
                    .newInstance(QFactory.getAttributeValue(l, "class"));
